...
class Game {
  ...
  constructor() {
    ...
    // --- DOM Elements ---
    /** @type {HTMLElement} The main game board container. */
    this.board = document.getElementById('board');
    /** @type {HTMLElement} The element displaying the current score/candies left. */
    this.candiesInfo = document.getElementById('candies');
    /** @type {HTMLElement} The element displaying the moves left. */
    this.movesInfo = document.getElementById('moves');
    /** @type {HTMLElement} The element displaying the current level. */
    this.levelInfo = document.getElementById('level');
    /** @type {HTMLElement} The image element for the required candy. */
    this.requiredCandyImage = document.getElementById('candy');
    /** @type {HTMLElement} The play button. */
    this.playButton = document.getElementById('button');
    // Other UI elements for titles
  ...
  setLevel() {
    ...
    // Initial check for matches on the new board
  ...
  removeEventListeners() {
      ...
      // We need to re-populate it with the new nodes.
  ...
  dragEnd(event) {
    ...
    if (this.replacedSquareId === null || this.draggedSquareId === null) return;
  ...
  findAndClearAllMatches() {
    ...
    // Check for matches of 3, 4, and 5 (could be expanded)
    for (let i = 0; i < this.width * this.width; i++) {
        // Rows
  ...
  /**
   * Moves existing candies down to fill empty spaces.
   */
  dropCandies() {
    for (let i = this.width * this.width - 1; i >= 0; i--) {
        if (this.squares[i].style.backgroundImage === '') {
            let firstNonEmptyAbove = -1;
            for (let k = i - this.width; k >= 0; k -= this.width) {
                if (this.squares[k].style.backgroundImage !== '') {
                    firstNonEmptyAbove = k;
                    break;
                }
            }
            if (firstNonEmptyAbove !== -1) {
                this.squares[i].style.backgroundImage = this.squares[firstNonEmptyAbove].style.backgroundImage;
                this.squares[firstNonEmptyAbove].style.backgroundImage = '';
            }
        }
    }
  }
  ...
  /**
   * Fills the top row with new random candies where empty.
   */
  refillBoard() {
    for (let i = 0; i < this.width; i++) {
        if (this.squares[i].style.backgroundImage === '') {
            const newColorIndex = Math.floor(Math.random() * this.colors.length);
            this.squares[i].style.backgroundImage = this.colors[newColorIndex];
        }
    }
  }
